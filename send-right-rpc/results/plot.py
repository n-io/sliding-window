...
for num_elem in num_elems:
  right_cycles = df.loc[df['num_elems'] == num_elem, 'right_cycles']
  kernel_width = df.loc[df['num_elems'] == num_elem, 'kernel_width']
  plt.plot(kernel_width, right_cycles, '-o', label=num_elem)

plt.legend(bbox_to_anchor=(1.05, 1.05))
plt.grid()
plt.xlabel('Kernel Width (Number of PEs)')
plt.ylabel('Right PE Cycles')
plt.xlim([0, max_kernel_width])
plt.ylim([0, max_right_cycles])

if not cs2:
  plt.title("Right Cycle Count, Simulation")
  plt.savefig('right_cycle_sim.png', bbox_inches='tight')
else:
  plt.title("Right Cycle Count, CS2")
  plt.savefig('right_cycle_cs2.png', bbox_inches='tight')
