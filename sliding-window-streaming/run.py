...
runner.memcpy_h2d(memcpy_h2d_color, data, kernel_width-1, 0, 1, 1, num_elems,
                  streaming=True, data_type=memcpy_dtype,
                  order=memcpy_order, nonblock=False)

print("make it here?")
# ...
left_out_data = np.zeros([num_elems], dtype=np.int16)
right_out_data = np.zeros([num_elems], dtype=np.int16)

runner.memcpy_d2h(left_out_data, memcpy_d2h_color, 0, 0, 1, 1, num_elems,
                  streaming=True, data_type=memcpy_dtype,
                  order=memcpy_order, nonblock=False)
