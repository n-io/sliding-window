...
def cslc_compile(
  ...
  args.append("--max-inlined-iterations=1000000")
...
def cs_run(
  ...
  try:
    subprocess.check_call(args, timeout=1200) # todo
  except:
    pass
...
def main():
  ...
  kernel_width=72
  num_elems = 500
  # for num_elems in range(100,4001,100):
  for kernel_width in range(8, 64*4+1, 4):
    compile_and_run(
        kernel_width,
        num_elems,
        name,
