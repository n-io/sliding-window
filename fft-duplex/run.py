...
def main():
    ...
    # runner = SdkRuntime(name, cmaddr=cmaddr, suppress_simfab_trace=True)
    ...
    arr2_symbol = runner.get_id("arr2")
    ...
    runner.memcpy_d2h(arr1_out_data, arr1_symbol, 0, 0, kernel_width, 1, num_elems,
                      streaming=False, data_type=memcpy_dtype,
                      order=memcpy_order, nonblock=False)
    print("Copied back arr1.")
    
    # Copy back data in arr1 from device
    arr2_out_data = np.zeros([kernel_width*num_elems], dtype=np.float32)
    runner.memcpy_d2h(arr2_out_data, arr2_symbol, 0, 0, kernel_width, 1, num_elems,
                      streaming=False, data_type=memcpy_dtype,
                      order=memcpy_order, nonblock=False)
    print("Copied back arr2.")
    ...
    all_times = [sdk_utils.calculate_cycles(t) for t in maxmin_time[0, :]]
    # print("AllTimes:    ", all_times)
    print(f"num_elems={num_elems} kernel_width={kernel_width} Best: {min(all_times)} worst: {max(all_times)} avg: {int(sum(all_times)/len(all_times))} all_times: {all_times}")
    print()
